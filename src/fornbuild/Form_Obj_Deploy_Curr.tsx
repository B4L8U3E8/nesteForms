...
const InpStyle:CSSProperties= {display: 'flex',
...
const Disp=(DO:OBJ)=>
    {
        return( Object.entries(DO).map(([key, val],i)=>{     
            if (typeof val === 'object')
                {   
                     return(     
                                <div key={key}   style= {{  borderBlockColor:`rgb(${i*150}, 80, 105)`, 
                                                            marginLeft:'20px'}} >    
                                    { Object.keys(val).length === 0  ? 
                                        <span style={{  display:"flex"}}> 
                                        <div> {key}:     </div>    
                                            <input  key={DynamicKey.current.toString()}
                                                    id='INP'
                                                    style={InpStyle}
                                                    type='text'
                                                    name={key}
        ...
        if (Array.isArray(OP))
            { return ( OP.map((v,k)=>{
                    ...
                    _____________________
                            <br/>
                                <span style={{color:'greenyellow', fontSize:'24px'}}> {k}:</span>
                                {Output(v)}
                        </div>
                        )
                }))
                ...
                if (typeof OP === 'object')
                    {return (
                        Object.entries(OP).map(([ke,va])=>
                        <div key={ke} style={{position:'relative', left:'20px'}}> 
                            <span> {ke} : </span>
                            <span> {typeof va ==='string' ? <span style={{fontSize:'24px'}}> {va} </span> : null}
                            </span>
                            {Output(va as OBJ)};                           
                        </div>
                        ))
    ...
    <div className="root">
        ...
        <div className="b_container">
            <div id='control_container' style={{alignItems:'center'}}>
                   <br/> 
                    <div style={{fontSize:'24px', color:'darkseagreen'}}> CREATE FORM</div>
                    <br/>
                        <button     id='btnA' 
                                    onClick={setValue}>         
                                    Inpur 
        ...
        <div className="b_container">
            ...
            <br/>
                ...
                <br/>
                KEY CHAIN {JSON.stringify(keyChain.current)}
                <br/>
            </div>
        </div>
